/**
 * 节点 IP 质量检测 · Quantumult X 任务脚本
 *
 * 使用:在 QX 的交互任务中执行，并通过 policy 参数指定策略组或节点。
 * ===== Quantumult X =====
 * [task_local]
 * event-interaction https://example.com/ipquality-qx.js#policy=Proxy&MaskIP=false&MediaTest=true&MapNotification=false, tag=节点 IP 质量检测, enabled=true
 *
 * policy 可以是策略组名或节点 tag；含空格或中文时请 URL 编码。
 */

const SCRIPT_VERSION = "2026-07-20.qx-r1";
...
const sourcePath = typeof $environment !== "undefined" && $environment.sourcePath
    ? String($environment.sourcePath)
    : "";
const runtimeArguments = parseArguments(sourcePath.indexOf("#") >= 0
    ? sourcePath.slice(sourcePath.indexOf("#") + 1)
    : "");
const policyName = cleanValue(runtimeArguments.policy || runtimeArguments.Policy
    || runtimeArguments.node || runtimeArguments.Node);
const nodeName = policyName || "当前规则";
...
function render(ip, data, media) {
    ...
    const content = renderTextReport(basic, types, risks, factors, media, audit, data._probe);

    postMapNotification(basic, displayNodeName);
    console.log(content);
    $notify("节点 IP 质量检测", displayNodeName, content);
    $done();
}

function renderTextReport(basic, types, risks, factors, media, audit, probe) {
    const lines = [basic.ip];
    if (basic.actualRegion) lines.push(basic.actualRegion);
    if (basic.city) lines.push(basic.city);
    if (basic.asn || basic.organization) {
        lines.push([basic.asn, basic.organization].filter(Boolean).join(" · "));
    }
    const basicRows = [
        ["类型", basic.nature],
        ["网段", basic.route],
    ...
    if (basicRows.length) {
        lines.push("", "基础信息");
        basicRows.forEach((row) => lines.push(`${row[0]}: ${row[1]}`));
    }
    if (types.length) {
        lines.push("", "IP 类型属性");
        types.forEach((row) => {
            ...
            lines.push(`${row.name}: ${details.join(" · ")}`);
        });
    }
    lines.push("", "风险评分");
    risks.forEach((row) => {
        lines.push(row.available
            ? `${riskSymbol(row.severity)} ${row.name}: ${[row.detail, row.label].filter(Boolean).join(" · ")}`
            : `⚪ ${row.name}: 未返回`);
    });
    if (factors.length) {
        lines.push("", "风险因素");
        factors.forEach((source) => {
            ...
            const details = [];
            if (hit.length) details.push(`命中 ${hit.join("、")}`);
            if (clear.length) details.push(`未命中 ${clear.join("、")}`);
            lines.push(`${source.name}${source.country ? ` [${source.country}]` : ""}: ${details.join("；")}`);
        });
    }
    lines.push("", "流媒体与 AI");
    if (!mediaEnabled) {
        ...
        media.forEach((row) => {
            const status = mediaStatus(row.status).text;
            lines.push(`${mediaSymbol(row.status)} ${row.name}: ${status}${row.region ? ` [${row.region}]` : ""}`);
        });
    }
    const probeText = probe && probe.total
        ? `；出口 ${probe.matched}/${probe.total}${probe.unique > 1 ? "，存在分流差异" : "，一致"}`
        : "";
    lines.push("", `数据状态: ${audit.success.length}/${audit.total}${probeText}`);
    if (audit.failed.length) lines.push(`未返回: ${audit.failed.join("、")}`);
    if (audit.skipped.length) lines.push(`已跳过: ${audit.skipped.join("、")}`);
    if (audit.supplemental.length) lines.push(`补充来源: ${audit.supplemental.join("、")}`);
    return lines.join("\n");
}

function riskSymbol(severity) {
    if (severity >= 3) return "🔴";
    if (severity >= 2) return "🟠";
    return "🟢";
}

function mediaSymbol(status) {
...
function buildBasic(ip, data) {
    ...
    const profile = profiles.find((item) => item.countryCode || item.countryName)
        ...
        || { source: "", cityParts: [] };
    const platformNetwork = platformNetworkProfile();
    const networkProfile = profiles.find((item) => item.asn)
        || (platformNetwork.asn ? platformNetwork : null)
        || profiles.find((item) => item.organization)
        || (platformNetwork.organization ? platformNetwork : null)
        || profile;
    const networkOrganization = cleanValue(networkProfile.organization)
        || (networkProfile.asn && networkProfile.asn === platformNetwork.asn
            ? cleanValue(platformNetwork.organization)
            : "");
...
function request(method, url, options) {
    ...
    return new Promise((resolve, reject) => {
        ...
        const requestPolicy = cleanValue(config.node) || policyName;
        if (requestPolicy) {
            requestOptions.opts.policy = requestPolicy.toUpperCase() === "DIRECT"
                ? "direct"
                : requestPolicy;
        }
        if (numberOrNull(config.timeout) !== null) {
            requestOptions.timeout = Math.max(1, Math.ceil(Number(config.timeout) / 1000));
        }
        ...
        $task.fetch(requestOptions).then((response) => {
            ...
            const body = response && typeof response.body !== "undefined" ? response.body : "";
...
function readSwitch(key, defaultValue) {
    if (Object.prototype.hasOwnProperty.call(runtimeArguments, key)) {
        return switchValue(runtimeArguments[key], defaultValue);
    }
...
function switchValue(value, defaultValue) {
    if (value === null || typeof value === "undefined" || value === "") return defaultValue;
...
function parseArguments(rawValue) {
    const result = {};
    String(rawValue || "").replace(/^[#?]/, "").split("&").forEach((pair) => {
        if (!pair) return;
        const separator = pair.indexOf("=");
        const rawKey = separator >= 0 ? pair.slice(0, separator) : pair;
        const rawItemValue = separator >= 0 ? pair.slice(separator + 1) : "true";
        try {
            result[decodeURIComponent(rawKey.replace(/\+/g, " "))] = decodeURIComponent(
                rawItemValue.replace(/\+/g, " ")
            );
        } catch (_) {
            result[rawKey] = rawItemValue;
        }
    });
    return result;
}
...
function platformNetworkProfile() {
    const result = {
        source: "Quantumult X",
        asn: "",
        organization: "",
...
function finishError(message) {
    console.log(message);
    $notify("节点 IP 质量检测", "执行失败", message);
    $done();
}
