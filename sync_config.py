...
FILTERS = {
 "香港": r"(?=.*(广港|香港|HK|Hong ?Kong|🇭🇰|HongKong)).*$",
 "台湾": r"(?=.*(广台|台湾|台灣|TW|Tai ?Wan|🇹🇼|TaiWan|Taiwan)).*$",
 "日本": r"(?=.*(广日|日本|JP|川日|东京|大阪|泉日|埼玉|沪日|深日|🇯🇵|Japan)).*$",
 "新加坡": r"(?=.*(广新|新加坡|SG|坡|狮城|🇸🇬|Singapore)).*$",
 "韩国": r"(?=.*(广韩|韩国|韓國|KR|首尔|春川|🇰🇷|Korea)).*$",
 "美国": r"(?=.*(广美|美国|US|纽约|波特兰|达拉斯|俄勒|凤凰城|费利蒙|拉斯|洛杉|圣何塞|圣克拉|西雅|芝加|🇺🇸|United ?States)).*$",
 "英国": r"(?=.*(英国|伦敦|UK|United ?Kingdom|🇬🇧|London)).*$",
}
